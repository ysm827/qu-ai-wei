...
**把 AI 生成的简体中文初稿改得自然一点，同时保留事实、正式程度和原文声口。**
...
没有信息的背景、强调和口号可以删；原文给出的版本数和问题数必须保留。
...
这类文字有具体经历、自嘲和自然节奏。qu-ai-wei 会判断为真人文本并停手，不把个人声口当成“AI 痕迹”清掉。

更多边界案例见 [`references/examples.md`](./references/examples.md)。

## 常见的中文 AI 味

下面是最容易被读者认出的 10 类结构。它们是检查线索，不是违禁词：单独出现一次通常不算问题；只有反复出现、脱离事实或让句子空转时才处理。

| 常见结构 | 什么时候值得改 | 什么时候要保留 |
|---|---|---|
| 随着……发展／在……背景下 | 开头只负责营造宏大感，删掉不影响信息 | 背景本身解释后文条件或时间 |
| 值得一提／不可否认 | 只强调，没有新增事实 | 确实承担转折、限定或作者判断 |
| 不是 X，而是 Y | 两端都是抽象评价，只把概念换个名字 | 用来区分退款与撤销等不同动作 |
...
完整的 51 类模式及语体开关见 [`SKILL.md`](./SKILL.md#51-类模式索引)；判定边界见 [`references/editing-boundaries.md`](./references/editing-boundaries.md)。
...
1. 先判断文本是否包含疑似凭证，以及原文是否已经像真人写的。
2. 识别社交、自媒体、商务、书面、学术、公文等语体，避免过度口语化。
3. 锁住数字、人物、时间、术语、引用和责任关系，不允许改写后漂移。
4. 按当前语体扫描 51 类 AI 写作模式，再处理动词、节奏、填充语和语序。
5. 回读终稿，检查事实、语体、具体细节和原文声口是否保住。

完整执行规则见 [`SKILL.md`](./SKILL.md)，短案例见 [`references/examples.md`](./references/examples.md)。

## 只要终稿

默认模式会给出门检、初稿、自审、终稿和打磨报告。如果 qu-ai-wei 只是工作流中的一步，可以要求它只返回终稿正文：
